...
- Right click the puzzle board and choose...
	...
	- "Cheat: Gather Pieces" to make a "mini puzzle" (a small subset of the larger puzzle) for a less skilled player to work on. Optionally, start by placing one puzzle piece on the puzzle board, then right-click the puzzle board and select this option. It will ask you for a target number of pieces; choose and confirm. After allowing other players to accept (perhaps they're already working on that section!), pieces (hopefully) near each other will be brought together for you. You now have a subset "circle" of puzzle to work on.
